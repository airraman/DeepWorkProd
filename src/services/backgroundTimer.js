...
// TEMP: Functions disabled for testing without Firestore
...
const sendCompletionNotification = async () => {
  try {
    ...
    debugLog('🔔 Sending FCM notification via HTTP for device:', deviceId.substring(0, 15) + '...');
    
    // ✅ Call Cloud Function via HTTP
    const functionUrl = 'https://us-central1-deepwork-8416f.cloudfunctions.net/triggerSessionEndNotification';
    ...
    try {
      ...
      if (response.ok) {
        debugLog('✅ FCM notification triggered via HTTP');
      } else {
        debugLog('⚠️ FCM HTTP request failed:', await response.text());
      }
    } catch (fetchError) {
